...
PKEVENT KeRequestCompleteEventObj;

static int sock;
static FILE_STANDARD_INFORMATION fileInfo = { 0 };
...
NTSTATUS CreateSharedEventsKe()
{
    ...
    uEventName.MaximumLength = 256;
    uEventName.Buffer = (PWSTR)symbolicNameBuffer;
    RtlUnicodeStringPrintf(&uEventName, L"%s", DRIVER_REQUESTCOMPLETEEVENT_NAME);
    InitializeObjectAttributes(&ObjectAttributes, &uEventName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL, NULL);
    eventHandle = IoCreateSynchronizationEvent(&uEventName, &hRequestCompleteEvent);

    if (eventHandle == NULL)
    {
        DbgPrint("CreateSharedEvents - DRIVER_REQUESTCOMPLETEEVENT_NAME Event fail!\r\n");
        return STATUS_UNSUCCESSFUL;
    }
    KeRequestCompleteEventObj = eventHandle;
...
)
{
    ...
    for (int i = 0; i < 1400; i++)
    {
        ...
        KeWaitForSingleObject(KeRequestCompleteEventObj, UserRequest, UserMode, FALSE, NULL);
        KeResetEvent(KeRequestCompleteEventObj);
...
//        KeSetEvent(KeRequestCompleteEventObj, 100, TRUE);
        KeDelayExecutionThread(KernelMode, FALSE, &timeout2);
