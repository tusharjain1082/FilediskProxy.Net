...
BOOL server2Steps()
{
    ...
    // phase 1: connect and receive request
    ...
    if (request->MajorFunction == IRP_MJ_READ)
    {
        ...
        ReadFile(file_handle, buffer, request->Length, NULL, NULL);
        fastBufferToPipe(buffer, pipe, 0, request->Length);
        //fastPipeToBuffer(&buffer, pipe, 0, request->Length);
