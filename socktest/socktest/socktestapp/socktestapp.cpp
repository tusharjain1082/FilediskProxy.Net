...
int main()
{
    ...
    for (int ctr = 0; ctr < 1400; ctr++)
    {
        ...
        //WaitForSingleObject(RequestCompleteEvent, -1);
...
void createEvents()
{
    ...
    // Event 3
    memset(link, 0, 512);
    sprintf_s(link, 512, "%s", USERMODEAPP_REQUESTCOMPLETEEVENT_NAME);
    RequestCompleteEvent = CreateEventA(NULL, TRUE, FALSE, link);
    if (RequestCompleteEvent == NULL)
    {
        std::cout << "RequestCompleteEvent create failed\r\n";
    }
